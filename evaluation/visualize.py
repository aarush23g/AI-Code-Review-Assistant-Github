...
def plot_radar_chart(metrics_data):
    ...
    labels = ['Detection Rate (Recall)', 'Precision', 'F1 Score', 'Avg Line Accuracy', 'Confidence Calibration (High/Very High)']
    num_vars = len(labels)
    ...
    for i, m in enumerate(metrics_data):
        ...
        ax.plot(angles, values, color=colors[i % len(colors)], linewidth=2, linestyle='solid', label=model_name)
...
def plot_cost_quality_tradeoff(metrics_data):
    ...
    for i, m in enumerate(metrics_data):
        ...
        ax.scatter(cost_per_review, f1_score, s=bubble_size, color=colors[i % len(colors)], 
                   alpha=0.6, label=f"{model_name}\n({latency_ms}ms avg latency)", edgecolors='w', linewidth=2)
        
        # Add labels to points
        ax.annotate(model_name, (cost_per_review, f1_score), xytext=(10, -5), 
                    textcoords='offset points', ha='left', va='bottom', fontsize=9, fontweight='bold')
    ...
    plt.xlim(0, max(m.get('cost_estimate', {}).get('cost_per_review_usd', 0) for m in metrics_data) * 1.5)
